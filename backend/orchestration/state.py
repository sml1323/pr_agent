...
class ReviewState(TypedDict):
    ...
    #
    # 결정 5 · `merged` 는 **새 채널**이고 **리듀서가 없다** (2026-08-28, M6-5)
    #
    #   · 결정 1 이 *"aggregate 의 산출물 자리도 안 팠다 — M6 의 일이다"* 라고 미뤄둔 칸이다.
    #     지금이 그 시점이다.
    #
    #   · **왜 findings 를 고쳐 쓰지 않나 — 못 한다. 실측:**
    #
    #         aggregate 가 {"findings": [합친 것]} 을 돌려주면
    #         → [원본 3개] + [합친 것 1개] = 4개.  **또 더해진다**
    #
    #     `operator.add` 가 붙은 채널에는 **덮어쓰기라는 동작 자체가 없다.**
    #     리듀서를 우회할 방법이 없으므로 채널을 가르는 건 취향이 아니라 **물리적 제약**이다.
    #
    #   · **제약이 이득을 하나 낳았다**: 원본 `findings` 가 그대로 남는다.
    #     "누가 무엇을 말했나"가 지워지지 않으므로 **애그리게이터가 뭘 버렸는지 추적된다.**
    #     `agent_events` 를 append-only 로 만든 이유(INV-4)와 같은 방향이다 —
    #     합친 결과만 남기면 M8 에서 "왜 이 지적이 사라졌나"에 답할 수 없다.
    #
    #   · **리듀서를 안 다는 것이 능동적 결정이다** (결정 2 와 같은 근거).
    #     이 칸에 쓰는 노드는 `aggregate` 하나뿐이다. 둘이 쓰면 터져야 맞다 —
    #     리듀서를 달아두면 M8 에서 게이트 노드가 실수로 여기 쓸 때
    #     **터지는 대신 두 배가 된 채 조용히 흘러간다.**
    #
    #   · ⚠️ 타입이 `list[dict]` 인 이유는 `findings` 와 같다 (결정 4) —
    #     체크포인터가 직렬화하므로 `MergedFinding` 객체를 그대로 담지 않는다.
    #     검증은 `aggregator.aggregate()` 안의 생성자가 이미 했다.
    #
    #   · ⚠️ `get_state()` 의 기본값 dict 에도 이 키를 넣어야 한다 (M5-7 결정) —
    #     안 넣으면 체크포인트가 0개일 때 호출부가 KeyError 를 본다.
    merged: list[dict[str, Any]]  # aggregate 만 쓴다 → 리듀서 없음
