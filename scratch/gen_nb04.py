"""노트북 04 생성기 — D3 배선: agent_type 을 계속 물어볼 것인가.

⚠️ 이 파일이 원본이다. 노트북 JSON 을 손으로 고치지 말 것.
⚠️ 학습자가 TODO 를 채운 뒤에는 재생성 금지 (작업이 날아간다).

실측 재료 (2026-08-28 측정, evals/runs/ 4파일 18판):
    finding 총 53개 중 agent_type 이 기록된 것 17개 (6판)
    17개 분포: security 7 · quality 6 · docs 2 · testing 2
    ⚠️ 그 6판은 전부 **에이전트 하나**가 낸 것이다 — 호출자는 넷으로 갈린 적이 없다.
"""
import json
from pathlib import Path

OUT = Path("learning/notebooks/04-agent-type-source.ipynb")
