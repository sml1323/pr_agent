...
**마지막 갱신**: 2026-08-27 · **M6-1 완료 — 자가 명령 한 줄이 됐다. 다음은 M6-4 배선 (또는 M6-3b)**
...
⬜ **`run` 은 실호출 미검증** — 프록시가 떠 있어야 하고 구독 한도를 쓴다.
`_usage_dict` 가 프록시의 `usage` 모양을 추측해서 짰다(없으면 건너뛰게 방어는 함).
**사용자가 직접 돌려야 하는 항목.**

⬜ **`scratch/prompt_2x2.json` 15판을 `evals/runs/` 로 이사 — M6-4 로 미룸.**
그 데이터엔 `tag_rule` 축이 있는데(`with_tag_rule`/`no_tag_rule`), 지금 `meta` 는 그 축을
**일부러 뺐다** — `review_diff()` 가 SYSTEM_PROMPT 를 하드코딩해서 넣을 구멍이 없기 때문이다.
M6-4 에서 그 구멍이 뚫리면 같은 모양으로 옮긴다.
