...
**마지막 갱신**: 2026-08-27 · **M6-3a 완료 — 프롬프트가 컴포넌트가 됐다 (`backend/prompts/`). 다음은 M6-0c(expected.yaml) 또는 M6-4 배선**
...
| **평가 코드 위치** | **최상위 `eval/`** (`backend/` 밖) | 구조 결정 카드 첫 적용 (2026-08-27). 근거: **평가는 프롬프트를 고르는 동안만 돈다 — 고르고 나면 프롬프트는 상수가 되어 배포되므로 평가 코드가 배포될 이유가 없다.** M8 게이트는 구간이 아니라 점(`confidence >= x`)을 보므로 `wilson_ci` 를 안 쓴다. ⚠️ **import 는 `eval/` → `backend/` 한 방향만** — 반대가 생기면 선을 넘은 것이고 그때 `backend/eval/` 로 이사한다 (비용: 디렉토리 이동 + import 몇 줄) |
